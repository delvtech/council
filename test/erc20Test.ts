...
describe("erc20", function () {
  ...
  before(async function () {
    ...
    const deployer = new MockERC20__factory(signers[0]);
    token = await deployer.deploy("token", "TKN", signers[0].address);
  ...
  describe("Permission-ed functions are permission-ed", async () => {
    it("Mints/burns with approved address", async () => {
      await token.mint(signers[2].address, 1);
      let balance = await token.balanceOf(signers[2].address);
      expect(balance).to.be.eq(1);
      await token.burn(signers[2].address, 1);
      balance = await token.balanceOf(signers[2].address);
      await token.burn(signers[2].address, 0);
    });
    it("Won't mint/burn from non owner address", async () => {
      let tx = token.connect(signers[1].address).mint(signers[2].address, 1);
      await expect(tx).to.be.revertedWith("Sender not owner");
      tx = token.connect(signers[1].address).burn(signers[2].address, 1);
      await expect(tx).to.be.revertedWith("Sender not owner");
    });
  });
});
