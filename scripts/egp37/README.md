...
```bash
npx hardhat run scripts/egp37/testCreateAndExecute.ts --no-compile --network hardhat
```
...
```bash
npx hardhat run scripts/egp37/executables/executablePreviewProposal.ts --no-compile --network mainnet
...
```bash
npx hardhat run scripts/egp37/executables/executableCreateProposal.ts --no-compile --network mainnet
```
...
```bash
 npx hardhat run scripts/egp37/executables/executableExecuteProposal.ts --no-compile --network mainnet
```
...
```bash
 npx hardhat run scripts/egp37/executables/executableExecuteTimelock.ts --no-compile --network mainnet
```