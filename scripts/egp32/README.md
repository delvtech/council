...
```bash
npx hardhat run scripts/egp32/testCreateAndExecute.ts --no-compile --network hardhat
```
...
```bash
npx hardhat run scripts/egp32/executables/executablePreviewProposal.ts --no-compile --network mainnet
...
```bash
npx hardhat run scripts/egp32/executables/executableCreateProposal.ts --no-compile --network mainnet
```
...
```bash
 npx hardhat run scripts/egp32/executables/executableExecuteProposal.ts --no-compile --network mainnet
```
...
```bash
 npx hardhat run scripts/egp32/executables/executableExecuteTimelock.ts --no-compile --network mainnet
```